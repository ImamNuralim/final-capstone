...
<div class="comment-container">
    <div class="comment  rounded">
                ...
                <div class="align-self-center">
                    <span class="avatar rounded-circle me-3" style="height: 45px; width: 45px">
                        ...
                        <img class="avatar rounded-circle"
                            ...
                            style="background-color: #DBE7F9; object-fit: cover; height: 100%; width: 100%">
                        @endif
                        @elseif ($answer->user->umum)
                        @if ($answer->user->umum->gambar)
                        <img class="avatar rounded-circle"
                            src="data:image/png;base64,{{ $answer->user->umum->gambar }}"
                            alt=""
                            style="background-color: #DBE7F9; object-fit: cover; height: 100%; width: 100%">
                        @else
                        <img class="avatar rounded-circle"
            ...
            <div class="flex-grow-1">
                ...
                <div>
                    <img class="img-fluid" style="max-height: 70px; max-width: 100%;"
                        ...
                        alt=""
                        data-bs-toggle="modal" data-bs-target="#answerimageModal{{ $answer->id_answer }}">
                </div>

                <!-- Modal -->
                <div class="modal fade" id="answerimageModal{{ $answer->id_answer }}" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 85vh!important;">
                        <div class="modal-content" style="background-color: unset; border: none;">
                            <div class="modal-body">
                                @if ($info->gambar)
                                <img src="data:image/png;base64,{{ $answer->gambar }}" class="img-fluid"
                ...
                <div class="d-flex justify-content-between align-items-center" style="margin-top: 5px;">
                    <div>
                        ...
                        <button id="replyTo-{{ $answer->id_answer }}"
                            ...
                            data-username="@if ($answer->user->ustaz) {{ $answer->user->ustaz->nama }} @elseif ($answer->user->murid) {{ $answer->user->murid->nama }} @elseif ($answer->user->umum) {{ $answer->user->umum->nama }} @else Nama tidak tersedia @endif"
                        ...
                        <div class="float-end">
                            ...
                            <div class="modal fade" id="editAnswerModal{{ $answer->id_answer }}" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
                                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <form id="editAnswerForm{{ $answer->id_answer }}" action="{{ route(auth()->user()->role . '.editanswer', $answer->id_answer ) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3 position-relative">
                                                    ...
                                                    <textarea name="deskripsi" id="editAnswerDeskripsi{{ $answer->id_answer }}" class="border rounded-0 form-control summernote" rows="6" placeholder="Deskripsi">{{ $answer->deskripsi }}</textarea>
                                                </div>
                                                <div class="mb-3 position-relative">
                                                    <label class="form-label" style="color: #000000;">Gambar</label>
                                                    <div id="editAnswerImageContainer{{ $answer->id_answer }}">
                                                        @if (!empty($answer->gambar))
                                                        <!-- Jika Gambar Sudah Ada -->
                                                        <div class="d-flex justify-content-center mb-3" onclick="document.getElementById('editAnswerFileInput-{{ $answer->id_answer }}').click();">
                                                            <img id="editAnswerImagePreview{{ $answer->id_answer }}" src="data:image/png;base64,{{ $answer->gambar }}" style="max-width: 100%; cursor: pointer;" alt="Gambar Jawaban">
                                                        </div>
                                                        <button type="button" id="deleteAnswerImageButton{{ $answer->id_answer }}" class="btn btn-danger btn-sm mb-3">Hapus Gambar</button>
                                                        @else
                                                        <!-- Jika Gambar Kosong -->
                                                        <div class="d-flex justify-content-start mb-3" onclick="document.getElementById('editAnswerFileInput-{{ $answer->id_answer }}').click();">
                                                    ...
                                                    <input type="file" id="editAnswerFileInput-{{ $answer->id_answer }}" name="image" style="display: none;" accept="image/*" onchange="editPreviewAnswerImage(event, '{{ $answer->id_answer }}')">
                                                    <input type="hidden" id="editAnswerActionInput{{ $answer->id_answer }}" name="action" value="">
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-ghost-secondary btn-pill" data-bs-dismiss="modal" id="editCancelAnswerButton{{ $answer->id_answer }}">Cancel</button>
                            ...
                            <script>
                                document.addEventListener('DOMContentLoaded', function() {
                                    const answerId = '{{ $answer->id_answer }}';
                                    const fileInput = document.getElementById('editAnswerFileInput-' + answerId);
                                    const previewContainer = document.getElementById('editAnswerImageContainer' + answerId);
                                    const deskripsiInput = document.getElementById('editAnswerDeskripsi' + answerId);
                                    const deleteButton = document.getElementById('deleteAnswerImageButton' + answerId);
                                    const modalElement = document.getElementById('editAnswerModal' + answerId);
                                    const answerActionInput = document.getElementById('editAnswerActionInput' + answerId);

                                    // Simpan nilai default
                                    const defaultDeskripsi = deskripsiInput.value;
                                    const defaultImageSrc = previewContainer.querySelector('img') ? previewContainer.querySelector('img').src : '';

                                    // Reset form saat modal ditutup
                                    modalElement.addEventListener('hide.bs.modal', function() {
                                        deskripsiInput.value = defaultDeskripsi;
                                        if (defaultImageSrc) {
                                            previewContainer.innerHTML = `
                    <div class="d-flex justify-content-center mb-3" onclick="document.getElementById('editAnswerFileInput-${answerId}').click();">
                        <img id="editAnswerImagePreview${answerId}" src="${defaultImageSrc}" style="max-width: 100%; cursor: pointer;" alt="Gambar Jawaban">
                    </div>
                    <button type="button" id="deleteAnswerImageButton${answerId}" class="btn btn-danger btn-sm mb-3">Hapus Gambar</button>
                `;
                                        } else {
                                            previewContainer.innerHTML = `
                    <div class="d-flex justify-content-start mb-3" onclick="document.getElementById('editAnswerFileInput-${answerId}').click();">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24">
                            <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1">
                                <path d="M15 8h.01M3 6a3 3 0 0 1 3-3h12a3 3 0 0 1 3 3v12a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3z" />
                                <path d="m3 16l5-5c.928-.893 2.072-.893 3 0l5 5" />
                ...
                `;
                                    ...
                                    if (deleteButton) {
                                        deleteButton.addEventListener('click', function() {
                                            deleteAnswerImagePreview(answerId);
                                        });
                                    }
                                });

                                // Fungsi untuk pratinjau gambar yang baru diunggah
                                function editPreviewAnswerImage(event, answerId) {
                                    ...
                                    if (file) {
                                        ...
                                        reader.onload = function(e) {
                                            ...
                                            previewContainer.innerHTML = `
                    <div class="d-flex justify-content-center mb-3" onclick="document.getElementById('editAnswerFileInput-${answerId}').click();">
                        <img id="editAnswerImagePreview${answerId}" src="${e.target.result}" style="max-width: 100%; cursor: pointer;" alt="Gambar Jawaban">
                    </div>
                    <button type="button" id="deleteAnswerImageButton${answerId}" class="btn btn-danger btn-sm mb-3">Hapus Gambar</button>
                `;
                                            document.getElementById('deleteAnswerImageButton' + answerId).addEventListener('click', function() {
                                                deleteAnswerImagePreview(answerId);
                                            });
                                        };
                                ...
                                function deleteAnswerImagePreview(answerId) {
                                    const previewContainer = document.getElementById('editAnswerImageContainer' + answerId);
                                    const answerActionInput = document.getElementById('editAnswerActionInput' + answerId);
                                    previewContainer.innerHTML = `
            <div class="d-flex justify-content-start mb-3" onclick="document.getElementById('editAnswerFileInput-${answerId}').click();">
        ...
        `;
                                    document.getElementById('editAnswerFileInput-' + answerId).value = '';
                                    answerActionInput.value = 'delete_image';
                                }
                            </script>
...
<script>
    ...
    function previewAnswerImage(event, questionId) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                // Membuat elemen preview gambar jika belum ada
                let previewContainer = document.getElementById('answerImagepreview-container-' + questionId);
                if (!previewContainer) {
                    previewContainer = document.createElement('div');
                    previewContainer.classList.add('d-flex', 'align-items-start', 'mb-3');
                    previewContainer.id = 'answerImagepreview-container-' + questionId;

                    previewContainer.innerHTML = `
                        <div>
                            <span class="avatar rounded-circle" style="background-color: unset; box-shadow: none;"></span>
                        </div>
                        <div class="flex-grow-1" id="answerImagepreview-${questionId}" style="margin-left: 10px; cursor: pointer;" onclick="document.getElementById('answerfileInput-${questionId}').click();"></div>
                    `;
                    const form = document.getElementById('answerForm-' + questionId);
                    form.insertBefore(previewContainer, form.querySelector('.d-flex.justify-content-end'));
                }
    ...
    function resetAnswerImage(questionId) {
        ...
        // Hapus elemen preview gambar jika ada
        const previewContainer = document.getElementById('answerImagepreview-container-' + questionId);
        if (previewContainer) {
